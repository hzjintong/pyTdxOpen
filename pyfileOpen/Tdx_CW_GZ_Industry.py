...
import warnings
from TechnicalAnalyzer2 import TechnicalAnalyzer
# import holidays
'''
...
class TDXFinancialValuationRanker:
    ...
    def __init__(self, cw_dir, day_data_dir, field_file="专业财务数据字段说明.txt", sector_file=None ):
        ...
        # 加载行业板块分类可用于股票名称映射
        self.code_to_name = {}
        self.code_to_industry_code = {}
        self.code_to_industry_name = {}
        if sector_file:
            self.load_stock_names(sector_file)

    def load_stock_names(self, sector_file):
        """加载板块文件，构建股票代码->名称、行业代码、行业名称的映射"""
        try:
            ...
            # 新增行业映射（注意行业代码和名称也要去除空格）
            self.code_to_industry_code = dict(zip(df['code'], df['Industry_code'].astype(str).str.strip()))
            self.code_to_industry_name = dict(zip(df['code'], df['Industry_name'].astype(str).str.strip()))
            print(f"已加载 {len(self.code_to_name)} 条股票名称、行业映射")
    ...
    def rank_by_category(self, years=5, top_n=100, category='综合财务', test_mode=False, by_industry=False):
        """
        按类别进行排名
        ...
        # 构建结果时增加行业分类信息，原代码中，当时为了性能和效率先截取了top_n再构建表格
        # for rank, (stock_code, score) in enumerate(sorted_stocks[:top_n], 1):
        # 改为处理所有有分数的股票， top_n 参数已移除
        for stock_code, score in sorted_stocks:  # 不截断，rank 先不赋值
            ...
            # 获取行业信息，若无则留空
            industry_code = self.code_to_industry_code.get(stock_code, '')
            industry_name = self.code_to_industry_name.get(stock_code, '')
        ...
        # 行业分组排名处理
        if by_industry and not df.empty :
            # 按行业分组，对每一个组内按得分降序排列，取前top_n名
            df['组内排名'] = df.groupby('行业代码')[f'{category}得分'].rank(ascending=False, method='min')
            df = df[df['组内排名'] <= top_n].copy()
            # 用组内排名替换原来的全局排名
            df['排名'] = df['组内排名'].astype(int)
            df.drop(columns=['组内排名'], inplace=True)
            # 按行业代码和排名排序
            df.sort_values(['行业代码', '排名'], inplace=True)
            df.reset_index(drop=True, inplace=True)
        else:
            # 若不需要行业分组，但已添加了行业列，可调整列顺序（将行业信息放在前面）
            cols = ['排名', '股票代码', '股票名称', '行业代码', '行业名称'] + \
                   [c for c in df.columns if c not in ['排名', '股票代码', '股票名称', '行业代码', '行业名称']]
            df = df[cols]
            # 全局排名（按得分降序给排名）
            df =df.sort_values(f'{category}得分', ascending=False)
...
def main():
    ...
    for category in categories_to_run:
        print(f"\n正在计算{category}排名...")
        df = ranker.rank_by_category(years=years, top_n=top_n, category=category, test_mode=test_mode, by_industry=True)
