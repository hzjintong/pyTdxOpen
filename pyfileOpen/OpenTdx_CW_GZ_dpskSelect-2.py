...
class TDXFinancialValuationRanker:
    ...
    def get_available_financial_files(self, look_back_years=5):
        """
        获取可用的财务数据文件
        根据当前日期自动选择合适的财务报告文件

        Args:
            look_back_years: 回溯多少年
        ...
        if not all_files:
            print(f"在目录 {self.cw_dir} 中没有找到财务数据文件")
            return []
        ...
        # 计算起始年份
        start_year = current_year - look_back_years
        ...
        for filename, date_str in all_files:
            try:
                ...
                if file_date < current_date:
                    available_files.append((filename, date_str))
            except Exception as e:
                print(f"判断当前日期选取文件日期错误{e}")
                continue

        # 按季度分组，每年最多4个季度文件
        quarterly_files = {}
        for filename, date_str in available_files:
            year = date_str[:4]
            month = date_str[4:6]
            ...
            if month in ['03', '04', '05']:
                ...
                quarter = f"{year}Q3"
            elif month in ['12', '01', '02']:  # 年报可能在12月或次年1-2月
                quarter = f"{year}Q4"
            else:
                continue

            # 每个季度只保留最新的文件
            if quarter not in quarterly_files or date_str > quarterly_files[quarter][1]:
                quarterly_files[quarter] = (filename, date_str)

        # 按季度排序，选择最新的几个季度
        sorted_quarters = sorted(quarterly_files.items(), key=lambda x: x[0], reverse=True)

        # 选择最近的几个季度文件（最多12个季度，即3年）
        selected_files = []
        for i, (quarter, (filename, date_str)) in enumerate(sorted_quarters):
            if i >= 12:  # 最多取3年的数据
                break
            selected_files.append((filename, date_str))

        # 按日期正序排列
        selected_files.sort(key=lambda x: x[1])

        return [os.path.join(self.cw_dir, f[0]) for f in selected_files]

    def get_latest_financial_file(self):
        """
        获取最新的财务数据文件
        ...
        Returns:
            最新的财务数据文件路径
        """
        files = self.get_available_financial_files(look_back_years=5)
        if files:
    ...
    def rank_by_category(self, look_back_years=3, top_n=100, category='综合', test_mode=False):
        """
        按类别进行排名

        Args:
            look_back_years: 回溯多少年的数据
            top_n: 显示前多少名
        ...
        """
        print(f"获取可用的财务数据文件...")
        files = self.get_available_financial_files(look_back_years)

        if not files:
            print("未找到可用的财务数据文件")
            # 尝试列出目录中的文件
            all_files = os.listdir(self.cw_dir)
            print(f"目录 {self.cw_dir} 中的文件:")
            for f in all_files[:10]:  # 只显示前10个文件
                print(f"  {f}")
            return pd.DataFrame()

        print(f"找到 {len(files)} 个可用的财务数据文件")
        for i, file_path in enumerate(files):
            filename = os.path.basename(file_path)
            date_str = filename[4:12]
            print(f"  {i + 1}. {filename} ({date_str[:4]}-{date_str[4:6]}-{date_str[6:8]})")

        # 使用最新的财务数据文件
        latest_file = files[-1]
        filename = os.path.basename(latest_file)
        date_str = filename[4:12]
        print(f"\n使用最新的财务数据文件: {filename} ({date_str[:4]}-{date_str[4:6]}-{date_str[6:8]})")
        ...
        try:
            with pd.ExcelWriter(filename, engine='openpyxl') as writer:
                ...
                for category, df in dfs_dict.items():
                    if not df.empty:
                        sheet_name = category[:30]  # Excel工作表名称不能超过31字符
...
def main():
    ...
    look_back_years = 5 if test_mode else int(input("回溯多少年的数据? (默认1): ") or "1")
    ...
    for category in categories_to_run:
        print(f"\n正在计算{category}排名...")
        df = ranker.rank_by_category(look_back_years=look_back_years, top_n=top_n, category=category,
                                     test_mode=test_mode)
...
def quick_ranking():
    ...
    print("\n获取可用财务数据文件...")
    files = ranker.get_available_financial_files(look_back_years=5)
    if not files:
        print("没有找到可用的财务数据文件")
        return
    ...
    print("\n正在计算综合财务与估值排名...")
    df_comprehensive = ranker.rank_by_category(look_back_years=1, top_n=30, category='综合', test_mode=True)
    ...
    print("估值水平排名前20名:")
    print("=" * 120)
    df_valuation = ranker.rank_by_category(look_back_years=1, top_n=20, category='估值', test_mode=True)
...
def analyze_single_stock():
    ...
    # 获取最新财务数据文件
    latest_file = ranker.get_latest_financial_file()
    if not latest_file:
        print("没有找到可用的财务数据文件")
        return

    print(f"使用财务数据文件: {os.path.basename(latest_file)}")
